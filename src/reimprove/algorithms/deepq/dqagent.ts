...
export class DQAgent extends AbstractAgent {
  ...
  infer(
    ...
    if (stateShifted) stateShifted.dispose();
  ...
  memorize(): void {
    ...
    if (this.forwardPasses <= <number>this.AgentConfig.temporalWindow + 1)
      return;
    ...
    return tidy(() => {
      ...
      if (!this.done) {
        target =
          ...
          (memento.reward +
            gamma *
              this.model.predict(memento.nextState.tensor).getHighestValue());
      }
  ...
  async learn(gamma: number, alpha: number) {
    const trainData = this.memory
      .sample(<number>this.AgentConfig.batchSize)
