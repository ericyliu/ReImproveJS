...
export class Academy {
  ...
  addTeacher(config?: TeachingConfig, name?: string): string {
    ...
    if (!teacher.Name) teacher.Name = v4();
   ...
   */
  async step(
    inputs: AcademyStepInput[] | AcademyStepInput
    ...
    for (let input of finalInput) {
      ...
      const agentsActions = await this.teachers
        .get(input.teacherName)
        .teach(input.agentsInput);
      agentsActions.forEach((value, key) => {
        if (actions.has(key))
          throw new Error(
            "Dqagent " + key + " has already registered an action."
          );
  ...
  OnLessonEnded(
    teacherName: string,
    callback: (teacher: string, lessonNumber: number) => void
  ) {
    if (this.teachers.has(teacherName))
  ...
  toggleTeaching(teacher: string, toggle: boolean): void {
    if (toggle === true) this.teachers.get(teacher).startTeaching();
    else this.teachers.get(teacher).stopTeaching();
  }
}
